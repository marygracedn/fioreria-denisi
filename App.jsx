...
// ── Supabase ──────────────────────────────────────────────────────────────────
const SB_URL = "https://dlmrusxlledgmtutxdcg.supabase.co";
const SB_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRsbXJ1c3hsbGVkZ210dXR4ZGNnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzc4OTE3NDgsImV4cCI6MjA5MzQ2Nzc0OH0.fZJ0I6-C5l5Z4C9GDLRLCQxdnWUQFJQMznqM99y81Nk";
const HDR = { "apikey": SB_KEY, "Authorization": `Bearer ${SB_KEY}`, "Content-Type": "application/json" };

const sbGet  = async (t, q="") => { try { const r = await fetch(`${SB_URL}/rest/v1/${t}?${q}`, { headers: HDR }); return r.ok ? r.json() : []; } catch { return []; } };
const sbIns  = async (t, d)    => { try { await fetch(`${SB_URL}/rest/v1/${t}`, { method:"POST", headers:{...HDR,"Prefer":"return=representation"}, body:JSON.stringify(d) }); } catch {} };
const sbDel  = async (t, id)   => { try { await fetch(`${SB_URL}/rest/v1/${t}?id=eq.${id}`, { method:"DELETE", headers: HDR }); } catch {} };
const sbSet  = async (k, v)    => { try { await fetch(`${SB_URL}/rest/v1/impostazioni`, { method:"POST", headers:{...HDR,"Prefer":"resolution=merge-duplicates"}, body:JSON.stringify({chiave:k,valore:v}) }); } catch {} };
const sbSets = async ()        => { try { const r = await sbGet("impostazioni","select=chiave,valore"); return Object.fromEntries((r||[]).map(x=>[x.chiave,x.valore])); } catch { return {}; } };
...
export default function FloralBooking() {
  ...
  const activePayments = () => {
    ...
    return [{id:"carta",icon:"💳",title:"Carta di credito / debito",sub:"Pagamento sicuro online"},
      ...
      .filter(m=>dp[m.id]!==false&&paymentMethods[m.id]);
  };
      ...
      <div style={{maxWidth:500,margin:"0 auto",padding:"13px 14px 40px"}}>
        ...
        {step==="noteGift"&&(
          <div style={{background:"#fff",borderRadius:20,padding:20,boxShadow:"0 4px 28px rgba(169,48,88,0.09)",border:"1px solid #f5c97e"}}>
            ...
            <button onClick={()=>setStep("form")} style={{...GB,width:"100%",marginTop:14,fontSize:14}}>Continua →</button>
            <button onClick={()=>setStep("service")} style={BB}>← Indietro</button>
          </div>
        )}
        ...
        {step==="form"&&(
          <div style={{background:"#fff",borderRadius:20,padding:20,boxShadow:"0 4px 28px rgba(169,48,88,0.09)",border:"1px solid #f5d5cc"}}>
            ...
            <button
              ...
              {(delivery==="acconia"||delivery==="dintorni")?"Continua →":"🌸 Conferma Prenotazione"}
            </button>
